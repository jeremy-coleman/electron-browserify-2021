...
const { convert, offsetSourceMaps } = require("./sm-helpers");
...
function LiveReactloadPlugin(b, opts = {}) {
  ...
  const server =
    opts.server !== false
      ? startServer({ port: Number(port), sslCert, sslKey })
      : null;
  let clientRequires = [];
  try {
  ...
  function addHooks() {
    ...
    const idToPath = id =>
      pathById[id] ||
      (_.isString(id) && id) ||
      throws("Full path not found for id: " + id);
    const idxToPath = idx =>
      pathByIdx[idx] ||
      (_.isString(idx) && idx) ||
    ...
    if (!dedupe) {
      ...
      if (b.pipeline.get("dedupe")) {
        log(
          "Other plugins have added de-duplicate transformations. --no-dedupe is not effective"
        );
      }
    } else {
      b.pipeline.splice(
      ...
      through.obj(
        function transform(row, enc, next) {
          ...
          if (converter) {
            ...
            adjustedSourcemap = convert
              .fromObject(offsetSourceMaps(converter.toObject(), 1))
              .toComment();
          } else {
            hash = getHash(source);
          }
    ...
    b.pipeline.get("wrap").push(
      through.obj(
        ...
        function flush(next) {
          const pathById = _.fromPairs(
            _.toPairs(mappings).map(([file, [s, d, { browserifyId: id }]]) => [
              id,
              file
            ])
          );
          ...
          const withFixedDepsIds = _.mapValues(
            mappings,
            ([src, deps, meta]) => [src, depsToPaths(deps), meta]
          );
        ...
        hot: {
          onUpdate: function(maybe, hook) {
            ...
            if (!realHook) {
              ...
              console.warn(
                "LiveReactload: You are providing two arguments to the module.hot.onUpdate hook, and we are" +
